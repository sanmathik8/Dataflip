...
S3_BUCKET = os.environ.get(
    "S3_BUCKET",
    "dataflip-analytics-dev"
)

GLUE_DATABASE = os.environ.get(
    "GLUE_DATABASE",
    "dataflip_db"
)
...
def schema_problems(
    ...
    for column in active_columns:
        ...
        elif new_types[name] != column["Type"]:
            problems.append(
                f"type of '{name}' changed "
                f"{column['Type']} -> {new_types[name]}"
            )
...
def delete_prefix(prefix):
    ...
    if not any(
        ...
        raise ValueError(
            f"refusing to delete non-slot prefix: {prefix}"
        )
...
def glue_active_slot(dataset):
    ...
    return slot_from_location(location)
...
def resolve_active_slot(
    ...
    manifest_slot = manifest.get(
        "active_slot"
    )

    glue_slot = glue_active_slot(
        dataset
    )

    if (
        glue_slot
        and glue_slot != manifest_slot
    ):

        logger.warning(
            "Manifest and Glue disagree for %s: "
            "manifest=%s glue=%s",
            dataset,
            manifest_slot,
            glue_slot
...
def set_glue_table_pointer(
    ...
    if not columns:
        raise ValueError(
            "refusing to write Glue table with no columns"
        )
    ...
    for attempt in range(
        ...
        try:
            ...
            table_input = {
                ...
                if key in GLUE_TABLE_INPUT_KEYS
            }
            ...
            glue.exceptions.ConcurrentModificationException,
    ...
    raise RuntimeError(
        f"Glue update for {dataset} "
        f"failed after {GLUE_RETRIES} attempts"
    )
...
def deploy_record(record):
    ...
    dataset, filename = parse_incoming_key(
        key
    )
    ...
    (
        ...
        rows,
        reason
    ) = validate_parquet(data)

    if (
        is_valid
        and ENFORCE_SCHEMA_COMPAT
        and active
    ):
...
def rollback(dataset):
    ...
    if previous not in SLOTS:
        ...
        return {
            "status": "error",
            "message": "no previous slot to roll back to"
        }
    ...
    previous_columns = previous_metadata.get(
        "columns",
        []
    )
    ...
    if not previous_columns:
        ...
        return {
            "status": "error",
            "message": (
                f"no schema recorded for "
                f"slot '{previous}'"
            )
        }

    if not prefix_has_objects(
        slot_prefix(
            dataset,
            previous
        )
